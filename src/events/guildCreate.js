...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`Apollo has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const ApolloColor = guild.roles.cache.find(r => r.name === '#Seagrass');
    if (ApolloColor) await guild.me.roles.add(ApolloColor);
  } catch (err) {
    client.logger.error(err.message);
  }
